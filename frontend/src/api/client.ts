...
export async function register(username: string, password: string) {
  ...
  if (!res.ok) {
    ...
    throw new Error(e.detail || 'Register failed');
  }
...
// === Games ===
export async function newGame(role: string = 'random') {
...
export async function fetchRecords() {
  ...
  return res.json() as Promise<{ records: Array<{ id: number; game_id: string; human_role: string; winner: string | null; total_rounds: number; is_alive: boolean; created_at: string }> }>;
}

export async function fetchRecordDetail(gameId: string) {
  const res = await fetch(API_BASE + '/records/' + gameId, {
    headers: { ...authHeaders() },
  });
