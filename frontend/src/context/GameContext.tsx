...
interface ContextValue {
  ...
  startGame: (role: string) => Promise<{ game_id: string; human_role: string; message: string }>;
...
export function GameProvider({ children }: { children: ReactNode }) {
  ...
  const startGame = useCallback(async (role: string) => {
    dispatch({ type: 'RESET' });
    const result = await newGame(role);
    dispatch({
      type: 'SET_STATE',
      payload: {
        ...INITIAL,
